  ...
  <div class="h-full w-full flex bg-white dark:bg-gray-900">
    <div class="w-full md:w-96 lg:w-[400px] h-full flex-shrink-0 border-r border-gray-200 dark:border-gray-800"
      ...
      <aside class="w-full h-full flex flex-col">
        <!-- WhatsApp-style header (match Chats) -->
        <header class="bg-emerald-600 dark:bg-emerald-700 text-white">
          <div class="px-4 py-3 flex items-center justify-between">
            <h2 class="text-lg font-semibold">Groups</h2>
            <div class="flex items-center gap-1">
              <button @click="openCreate" class="p-2 hover:bg-emerald-500 dark:hover:bg-emerald-600 rounded-full transition-colors" title="New group">
          ...
          <div class="px-3 pb-3">
            <div class="relative">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-200/90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Tabs: Chats | Groups (placed below header like Chats) -->
        <div class="px-4 pt-2 pb-1 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex gap-2">
          <router-link to="/" class="text-xs px-3 py-1 rounded-full"
                       :class="$route.path==='/' ? 'bg-emerald-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'">
            Chats
          </router-link>
          <router-link to="/groups" class="text-xs px-3 py-1 rounded-full"
                       :class="$route.path.startsWith('/groups') ? 'bg-emerald-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'">
            Groups
          </router-link>
        </div>
        <div class="flex-1 overflow-y-auto">
          <button v-for="g in filteredGroups" :key="g._id"
                  ...
                  class="w-full text-left px-4 py-3 hover:bg-emerald-50 dark:hover:bg-gray-800/50 flex items-center gap-3 transition-colors border-b border-gray-100 dark:border-gray-800/50 relative group">
            <img :src="avatar(g.avatarUrl)" class="w-12 h-12 rounded-full object-cover"/>
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <span class="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">{{ g.name }}</span>
                <span v-if="unread(g)" class="ml-auto bg-emerald-600 text-white text-xs px-2 py-0.5 rounded-full">{{ unread(g) }}</span>
              </div>
    ...
    <div class="w-full h-full flex flex-col" :class="!groups.activeGroupId ? 'hidden md:flex' : 'flex'">
      <div v-if="!groups.activeGroupId" class="flex-1 flex items-center justify-center">Select a group</div>
      <template v-else>
        <!-- Match Chat header component -->
        <div class="relative">
        ...
        <div ref="messagesContainer" class="flex-1 overflow-y-auto px-3 md:px-4 py-2 space-y-1" :style="{ backgroundColor: isDark ? '#0b141a' : '#efeae2' }">
    ...
    <div v-if="showCreate" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-lg mx-4">
        <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
          <div class="font-semibold">Create Group</div>
          <button @click="closeCreate" class="text-gray-500 hover:text-gray-700">✕</button>
        </div>
        <div class="p-4 space-y-4">
          ...
          <div>
            <label class="block text-sm mb-2">Select members</label>
            <div class="relative mb-2">
              <input v-model="createQuery" class="w-full input" placeholder="Search contacts" />
            </div>
            <div class="max-h-60 overflow-y-auto border rounded border-gray-200 dark:border-gray-800 divide-y divide-gray-200 dark:divide-gray-800">
...
async function confirmCreate() {
  ...
  const payload: any = { name: createName.value.trim(), members: selectedMembers.value }
