<template>
  <div class="max-w-[85%] md:max-w-[75%] mb-3 flex" :class="fromMe ? 'ml-auto justify-end' : 'justify-start'">
    <div class="relative inline-block max-w-full">
      ...
      <div class="px-4 py-2 rounded-2xl shadow-sm" 
        ...
        <!-- Text Message -->
        <div v-if="type==='text'" class="text-sm leading-relaxed whitespace-pre-wrap break-words">
          {{ content }}
        </div>
        
        <!-- Image Message -->
        <div v-else-if="type==='image'" class="rounded-xl overflow-hidden">
          <img v-if="!imageError"
               ...
               alt="Sent image" />
          <div v-else class="max-h-80 max-w-full w-auto bg-gray-100 dark:bg-gray-700 rounded-xl flex items-center justify-center p-4">
        ...
        <a v-else 
           ...
           target="_blank" 
           class="flex items-center gap-3 px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
        ...
        <div class="text-xs mt-1 flex items-center gap-1 justify-end" 
             :class="fromMe ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'">
          <span>{{ time }}</span>
          <!-- Read receipt for sent messages -->
          <svg v-if="fromMe" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
...
const props = defineProps({ fromMe: Boolean, type: String, content: String, time: String })
