...
export const useGroupsStore = defineStore('groups', {
  ...
  actions: {
    ...
    async fetchMessages(groupId, before) {
      ...
      this.messagesByGroup[groupId] = merged.sort((a, b) => 
          ...
          if (me && message.senderId !== me) {
            ...
            if (isActive) {
              ...
              if (this.socket?.connected) {
                this.socket.emit('group:markRead', { groupId: message.groupId })
              }
            } else {
    ...
    async setActiveGroup(groupId) {
      if (this.activeGroupId && this.socket?.connected) {
        this.socket.emit('leave_group', { groupId: this.activeGroupId })
      }
