...
export const useChatStore = defineStore('chat', {
      state: () => ({
            ...
            socket: null,
            users: [],
            webrtc: { pc: null, localStream: null, remoteStream: null, inCallWithUserId: null, incomingFromUserId: null, incomingOffer: null, muted: false, status: 'idle' }
      }),
      actions: {
            async bootstrap() {
            ...
            async fetchContacts() {
                  try {
                        ...
                        this.users = data || []
                        console.log('📋 Fetched contacts:', data?.length, 'users:', data?.map(u => ({ id: u._id, name: u.name })))
                        console.log('📋 Users data:', data?.map((u) => ({ id: u._id, name: u.name, username: u.username })))
            ...
            async fetchConversations() {
                  try {
                        ...
                        console.log('💬 Conversations data:', data?.map((c) => ({ id: c._id, participants: c.participants })))
                  } catch (error) {
            ...
            async openOrCreateConversation(participantId) {
                  ...
                  console.log('🔍 openOrCreateConversation:', {
                        ...
                        allConversations: this.conversations.map(c => ({
                              ...
                              isMatch: c.participants.length === 2 && c.participants.includes(participantId) && c.participants.includes(currentUserId)
                        }))
                  })
                  ...
                  try {
                        ...
                        socket.on('ice_candidate', async ({ fromUserId, candidate }) => {
                              ...
                              if (this.webrtc.pc && (this.webrtc.inCallWithUserId === fromUserId || this.webrtc.incomingFromUserId === fromUserId)) {
                                    try { await this.webrtc.pc.addIceCandidate(candidate) } catch (e) { console.error('addIceCandidate error', e) }
                              }
                        })
            ...
            async setupPeerConnection(peerUserId) {
                  ...
                  pc.onicecandidate = (e) => {
                        if (e.candidate && this.socket?.connected) {
                              this.socket.emit('ice_candidate', { toUserId: peerUserId, candidate: e.candidate })
                        }
                  }
            ...
            toggleMute() {
                  ...
                  this.webrtc.localStream?.getAudioTracks().forEach(t => t.enabled = !this.webrtc.muted)
            },
            endCall() {
                  try { if (this.webrtc.inCallWithUserId) this.socket?.emit('end_call', { toUserId: this.webrtc.inCallWithUserId }) } catch { }
                  this.webrtc.pc?.getSenders().forEach(s => { try { s.track?.stop() } catch { } })
                  this.webrtc.localStream?.getTracks().forEach(t => { try { t.stop() } catch { } })
                  try { this.webrtc.pc?.close() } catch { }
                  this.webrtc = { pc: null, localStream: null, remoteStream: null, inCallWithUserId: null, incomingFromUserId: null, incomingOffer: null, muted: false, status: 'idle' }
            },
            ...
            async acceptIncomingCall() {
                  try {
                        ...
                        if (answer) {
                              ...
                              this.socket?.emit('answer_call', { toUserId: fromUserId, answer })
            ...
            rejectIncomingCall() {
                  try {
                        if (this.webrtc.incomingFromUserId) {
                              this.socket?.emit('end_call', { toUserId: this.webrtc.incomingFromUserId })
                        }
                  } finally {
                        this.webrtc.incomingFromUserId = null
                        this.webrtc.incomingOffer = null
            ...
            async sendText(conversationId, to, content) {
                  try {
                        ...
                        if (this.socket?.connected) {
                              console.log('📤 Sending message via Socket.io:', { conversationId, to, content })
                              this.socket.emit('message:send', { conversationId, to, type: 'text', content })
                        } else {
                              console.log('⚠️ Socket not connected, falling back to HTTP')
                              // Fallback to HTTP if Socket.io is not available
                              const { data } = await api.post(`/conversations/${conversationId}/messages`, { to, type: 'text', content })
                        ...
                        if (e?.response?.data?.code === 'BLOCKED') alert('You are blocked by this user')
